...
const getProducts = asyncHandler(async (req, res) => {
  ...
  const [products, total] = await Promise.all([
    Product.find(filter).sort(sortBy).skip(skip).limit(limitNum).select("-reviews"),
...
const createProduct = asyncHandler(async (req, res) => {
  const {
    name, description,additionalInformation, price, comparePrice, category, brand,
...
const deleteProductImage = asyncHandler(async (req, res) => {
  ...
  await product.save();
  res.status(200).json({ success: true, message: "Image deleted", data: product.images });
});
...
const addReview = asyncHandler(async (req, res) => {
  ...
  res.status(201).json({ success: true, message: "Review added", data: product.ratings });
});
